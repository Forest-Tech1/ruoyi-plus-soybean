...
• Job No → 入库计划 systemSoNo；单元格内多个 SO 若换行或空格分行，规范化后合并为英文逗号分隔的一个字段
...
• Hold 列：单元格包含子串 HOLD（不区分大小写）→ deliveryMethod =「暂扣」；否则 →「卡车派送」
...
主单其它字段（提单号等）若表中无列，可由后端留空或按现有默认规则填充；导入模板建议与上表列名一致。`,
        ...
        inboundPlanRowNeedIdentifier: '第 {index} 行请至少填写「系统 SO 号」或「货件编码」之一',
