...
defineOptions({
  name: 'WarehouseLocationImportModal'
});
...
function handleFinish(options: { file: UploadFileInfo; event?: ProgressEvent }) {
  ...
  // @ts-expect-error ProgressEvent target may be XMLHttpRequest without full typings
...
function handleDownloadTemplate() {
  download(
    '/wms/inventory/location/importTemplate',
    {},
    `${$t('page.wms.inventory.location.title')}_${$t('common.importTemplate')}_${new Date().getTime()}.xlsx`
  );
}
...
<template>
  ...
  >
    <NUpload
      ...
      :action="`${baseURL}/wms/inventory/location/importData`"
      :headers="headers"
      :data="data"
      :max="1"
      :file-size="50"
